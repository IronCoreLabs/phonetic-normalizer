/*
* Much of this file is either directly copied or inspired by the ascii_folding_filter
* code in tantivy:
* https://github.com/tantivy-search/tantivy/blob/main/src/tokenizer/ascii_folding_filter.rs
* That code in turn is adapted from the similar code in Lucene:
...
* The Tantivy code is licensed under the MIT License and it is copyright by
* the AUTHORS of tantivy.
* Code copied on 2021-09-10. File last edited on 2021-07-01.
* Each function is individually labeled as well.
*/

// This function is inspired by the to_ascii function in
// tantivy-search's ascii_folding_filter
...
// This function is fully copied from tantivy-search's ascii_folding_filter
...
mod tests {
    ...
    // These tests are ours
    #[test]
    fn ascii_fold_specific_cases() {
        assert_eq!(&latinize("Noël"), "Noel");
        assert_eq!(&latinize("René"), "Rene");
        assert_eq!(&latinize("Adrián"), "Adrian");
        assert_eq!(&latinize("Zoë"), "Zoe");
        assert_eq!(&latinize("François"), "Francois");
        assert_eq!(&latinize("Mónica"), "Monica");
        assert_eq!(&latinize("Mathéo"), "Matheo");
        assert_eq!(&latinize("café"), "cafe");
        assert_eq!(&latinize("façade"), "facade");
        assert_eq!(&latinize("piña colada"), "pina colada");
        assert_eq!(&latinize("attaché"), "attache");
        assert_eq!(&latinize("cliché"), "cliche");
        assert_eq!(&latinize("naïve"), "naive");
        assert_eq!(&latinize("São Paulo"), "Sao Paulo");
        assert_eq!(&latinize("résumé"), "resume");
        assert_eq!(&latinize("Æneid"), "AEneid");
    }

    // I did not copy the test_all_foldings test from tantivy since it's thousands
    // of lines long and it is mostly a duplicate of the fold_non_ascii_char
    // function with the u-codes turned into actual characters.
}
